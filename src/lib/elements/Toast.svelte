<script context="module">
	 ...
	 * @prop {import('@floating-ui/dom').OffsetOptions} offsetOptions
	 */
	...
	export const notice = {
		...
		async setNotice(message, args) {
			const { type, autoHide, root, offsetOptions } = {
				...{
					type: 'success',
					autoHide: true,
					root: undefined,
					offsetOptions: args.root
						? {
								mainAxis: 16,
								crossAxis: 0
							}
...
<script>
	...
	import { CircleCheck, Info, TriangleAlert, X, XCircle } from '@lucide/svelte';
	...
	export { staticRoot as root };
	let className = '';
	export { className as class };
	/** @type {import('@floating-ui/core').Placement} */
	export let placement = 'bottom';
	/** @type {import('@floating-ui/dom').OffsetOptions} */
	let staticOffsetOptions = {
		mainAxis: 80,
		crossAxis: 80
	};
	export { staticOffsetOptions as offsetOptions };
	...
	$: root = $notice?.root ?? staticRoot;
	$: offsetOptions = $notice?.offsetOptions ?? staticOffsetOptions;

	$: if (wrapper && $notice && root) {
		cleanup();
		updatePosition();
		...
		{#key $notice.id}
			<div class={`item ${$notice.type}`} in:fly={inFly}>
				{#if icons.has($notice.type)}
					<span class="icon"><svelte:component this={icons.get($notice.type)} size="20" /></span>
				{/if}
				<div class="content">
					<span>{$notice.message}</span>
				</div>
				<button class="close" on:click={() => notice.reset()}><X /></button>
			</div>
		{/key}
	</section>
{/if}
